 ...
 * <b>lengthFieldLength</b>   = <b>2</b> 表示从第0个字节第1个字节拼起来的十进制数表示数据包长度，
 * lengthAdjustment    = 0               表示有时候长度域不能完全代表数据包长度，
 * initialBytesToStrip = 0 (= do not strip header) 解析数据包时候，需不需要跳过字节，
 *    Length 表示12，后面紧跟着就是12个字节。
 ...
 * <pre>
 * lengthFieldOffset   = 0       表示长度域偏移量是0，从第0个字节开始
 * lengthFieldLength   = 2      表示从第0个字节第1个字节拼起来的十进制数表示数据包长度，
 * lengthAdjustment    = 0    表示有时候长度域不能完全代表数据包长度，
 * <b>initialBytesToStrip</b> = <b>2</b> (= the length of the Length field) 解析数据包时候，需不需要跳过字节，
 *
 ...
 * lengthFieldOffset   =  0
 * lengthFieldLength   =  2
 * <b>lengthAdjustment</b>    = <b>-2</b> (= the length of the Length field) 往后数12个字节，
 ...
 */
// 如果消息是通过长度进行区分的，这个可以自动处理粘包和半包问题
 ...
 *
 * 参数组合进行解码
 *  lengthFieldOffset   =  0  长度域的偏移量，解决长度字段不在消息头的首位，当标识消息长度的字段位于消息头的中间或者尾部
 *  lengthFieldLength   =  2
 *  lengthAdjustment    =  0 ,长度字段放在消息头的首位；若长度仅用来标识消息体的长度，则默认0, 若长度还包括了消息头的长度，则要使用这个来修正。
 *  initialBytesToStrip =  0
 *
...
public class LengthFieldBasedFrameDecoder extends ByteToMessageDecoder {
    ...
    private void discardingTooLongFrame(ByteBuf in) {
        //process 判断需要丢弃的字节长度，由于丢弃的字节数不能大于当前缓冲区可读的字节数。
    ...
    protected Object decode(ChannelHandlerContext ctx, ByteBuf in) throws Exception {
        ...
        //process 说明当前可读字节流还没到达长度域结尾
        if (in.readableBytes() < lengthFieldEndOffset) {
            return null;
        }

        //process 表示长度域在实际二进制流中真实的绝对位置的偏移量
        int actualLengthFieldOffset = in.readerIndex() + lengthFieldOffset;
        //process  从字节流里从实际偏移量开始中读取legnth个字节数据
        long frameLength = getUnadjustedFrameLength(in, actualLengthFieldOffset, lengthFieldLength, byteOrder);

        if (frameLength < 0) { //process frameLength 小于0，不合法，抛出异常
