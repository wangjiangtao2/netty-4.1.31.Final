...
public abstract class AbstractByteBuf extends ByteBuf {
    ...
    /**
     * 所有的ByteBuf实例共享同一个ResourceLeakDetector对象，用于检测对象是否泄露
     */
    ...
    /**
     *首先对读索引进行判断，如果为0，则说明没有可重用的缓冲区，直接返回，
     * 如果读索引大于0且读索引不等于写索引，说明缓冲区中既有已经读取过的被丢弃的缓冲区，也有尚未读取的可读缓冲区。
     * 调用setBytes(0, this.readerIndex, writerIndex-readerIndex) 进行字节数组复制。
     * 将尚未读取的字节数组复制到缓冲区的起始位置，然后重新设置读写索引，读索引设置为0 ，写索引设置为之前的写索引减去读索引(重用的缓冲区长度)
     */
    @Override
    ...
    public ByteBuf readBytes(byte[] dst, int dstIndex, int length) {
        //对缓冲区的可用空间进行校验
        checkReadableBytes(length);
        //从当前的读索引开始，复制length个字节到目标byte数组中。由于不同子类复制操作的技术实现细节不同，因此由子类实现。
     ...
     * 对缓冲区的可用空间进行校验
     * 如果读取的长度小宇0，则抛出IllegalArgumentException异常提示参数非法
     * 如果可写的字节数小于需要读取的长度，则抛出IndexOutOfBoundsException异常。
     */
