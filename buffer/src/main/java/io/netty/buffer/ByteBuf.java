...
public abstract class ByteBuf implements ReferenceCounted, Comparable<ByteBuf> {
     ...
     * 从readerIndex 开始获取24位整型值，readerIndex增加3(注意：该类型并非java的基本类型，大多数场景使用不到)
     */
     ...
     * @throws IndexOutOfBoundsException
     *         if {@code this.readableBytes} is less than {@code 3}
     *  从readerIndex 开始获取24位无符号整型值，readerIndex增加3(注意：该类型并非java的基本类型，大多数场景使用不到)
     */
     ...
     *  从readerIndex开始获取整型值，readerIndex增加4
     */
     ...
     * @throws IndexOutOfBoundsException
     *         if {@code this.readableBytes} is less than {@code 4}
     *从readerIndex开始获取无符号整型值，readerIndex增加4
     */
     ...
     * 从readerIndex开始获取无符号长整型值，readerIndex增加8
     */
     ...
     * @throws IndexOutOfBoundsException
     *         if {@code this.readableBytes} is less than {@code 2}
     * 从readerIndex开始获取字符值，readerIndex增加2
     */
     ...
     * @throws IndexOutOfBoundsException
     *         if {@code this.readableBytes} is less than {@code 4}
     *  从readerIndex开始获取浮点值，readerIndex增加4
     */
     ...
     * @throws IndexOutOfBoundsException
     *         if {@code this.readableBytes} is less than {@code 8}
     *  从readerIndex开始获取双精度浮点值，readerIndex增加8
     */
     ...
     * @throws IndexOutOfBoundsException
     *         if {@code length} is greater than {@code this.readableBytes}
     *  将当前ByteBuf中的数据读取到新创建的ByteBuf中，读取的长度为length。操作成功完成之后，返回的ByteBuf的readerIndex为0，writerIndex为length
     *  如果读取的长度length大于当前操作的ByteBuf的可写字节数，将抛出IndexOutOfBoundsException，操作失败
     */
     ...
     * @throws IndexOutOfBoundsException
     *         if {@code length} is greater than {@code this.readableBytes}
     *  返回当前ByteBuf新创建的子区域，子区域与原ByteBuf共享缓冲区，但是独立维护字节的readerIndex和writerIndex
     ...
     * 将当前ByteBuf中的数据读取到目标ByteBuf中，直到目标ByteBuf没有剩余的空间可写，
     * 操作完成之后，当前ByteBuf的readerIndex+=读取的字节数
     *  如果目标ByteBuf可写的字节数大于当前ByteBuf可读的字节数则抛出IndexOutOfBoundsException，操作失败
     */
     ...
     *         if {@code length} is greater than {@code dst.writableBytes}
     *  将当前ByteBuf中的数据读取到目标ByteBuf中，读取的字节数长度为length
     *  操作完成之后，当前ByteBuf的readerIndex+=length
     *  如果需要读取的字节数长度length大于当前ByteBuf可读的字节数或者目标ByteBuf可写的字节数则抛出IndexOutOfBoundsException，操作失败
     */
