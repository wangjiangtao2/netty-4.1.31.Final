 ...
 *
 * 主要负责Nio相关部分，主要就是使用selector方式来进行读写事件的监听，
 * 客户端channel向它注册一个READ事件，服务端channel向它注册一个ACCEPT事件；
 * 服务端和客户端共同部分都是由它来实现。
 *
 *  doRegister() 方法注册
 *  doBeginRead() 准备处理读操作之前需要设置网络操作位为读
 */
public abstract class AbstractNioChannel extends AbstractChannel {
    ...
    /**
     * 服务端一开始创建的时候默认是 SelectionKey.OP_ACCEPT
     * 客户端一开始创建的时候默认是 SelectionKey.OP_READ
