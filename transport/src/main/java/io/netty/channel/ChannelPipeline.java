 ...
 * after the exchange.
 *
 * 是ChannelHandler的管理容器，负责ChannelHandler的查询、添加、替换和删除
 *
 *  ChannelPipeline内部是一个双向的链表结构，内部由一个个的ChannelHandlerContext节点组成，ChannelPipeline有头尾两个固定的节点HeadContext与TailContext。
 * 用户自定的ChannelHandler就是由ChannelHandlerContext包装成Pipeline的节点，参与Channel整个生命周期中所触发的入站事件与出站事件以及相应数据流的拦截处理。
 */
